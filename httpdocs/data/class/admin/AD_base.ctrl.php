...
class AD_base {
	...
	function insert( $arrVal, $arrSql = null ) {
		...
		if( !empty( $arrVal["base_category"] ) && is_array( $arrVal["base_category"] ) ){
			$arrVal["base_category"] = implode( ",", $arrVal["base_category"] );
		}
	...
	function update( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk . " = " . $arrVal["id_base"];
	...
	function GetIdRow( $id ) {
		...
		// データ取得
		$res = $this->_DBconn->selectCtrl( $creation_kit, array( "fetch" => _DB_FETCH ) );
		if( !empty( $res["base_category"] ) ){
					$res["base_category"] = explode( ",", $res["base_category"] );
				}
...
/*
	...
	function GetOption() {
		...
		// SQL配列
		$creation_kit = array(  "select" => "base_category, title",
		...
		if( !empty($arr_option) ){
			foreach( $arr_option as $val ){
				$res[$val["base_category"]] = $val["title"];
			}
		}
