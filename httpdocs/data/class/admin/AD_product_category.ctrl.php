...
class AD_product_category {
	...
	// 主テーブル
	var $_CtrTable   = "t_product_category";
	var $_CtrTablePk = "id_product_category";

	// コントロール機能（ログ用）
	var $_CtrLogName = "製品カテゴリマスタ";
	...
	function check( &$arrVal, $mode ) {
		...
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "カテゴリID", "all", $arrVal["id_product_category"], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
	...
	function update( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk . " = " . $arrVal["id_product_category"];
