...
class DB_manage extends DB_master {
	...
	function insertLog( $id, $bid, $log ) {
		...
		// ログ操作クラス読み込み
		include_once( _DOCUMENT_ROOT . "/data/class/DB_log.ctrl.php" );
	...
	// 内  容：連想配列の作成を行う。
	//-------------------------------------------------------
	function getDataArray( $creation_kit = NULL, $cKey, $cVal ) {
	...
	//-------------------------------------------------------
	function getCacheDataArray( $creation_kit = NULL, $cKey, $cVal, $cacheName ) {
		...
		if( !file_exists( $cacheDir ) ) {
			...
			// データ取得
			$res = $this->getDataArray( $creation_kit, $cKey, $cVal );
