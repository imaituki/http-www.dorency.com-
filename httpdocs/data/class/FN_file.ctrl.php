...
//********************************************************************************
	...
	// サポーター募集専用
	function copyImage2( $arrFiles, $arrImage, &$arrVal ) {
		...
		// 拡張子取得
		$explode_files = explode( ".", $arrFiles["name"] );
		$ext = ".". $explode_files[1];
		...
		foreach( $arrImage as $key => $val ) {
			...
			if( is_array( $val["option"] ) ) {
				...
				// 写真コピー
					...
					// ファイルパス
					$res[$val["name"]]["newImage"]["fileName"] = $arrVal["fileName"];
					$res[$val["name"]]["newImage"]["savePath"] = $val["option"]["l"]["savePath"] . $val["option"]["l"]["prefixFileName"] . $res[$val["name"]]["newImage"]["fileName"]. $ext;
					$res[$val["name"]]["inputPath"] = $arrFiles["tmp_name"];
